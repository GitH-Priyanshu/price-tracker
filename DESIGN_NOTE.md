...
1. **Fabricated Benchmark Rows & Failure Explanations**:
   ...
   *Correction*: Established the rule to report only from raw execution files. Every benchmark number is copied directly from saved JSON evidence.
2. **Polluting Live Database with Fake Test Rows**:
   ...
   *Correction*: Replaced live database mutation with an injected product loader and created test guards (`test-` prefix requirement plus an unforgeable Symbol token).
3. **Layered Price Truncation Bugs**:
   ...
   *Correction*: Captured real failing DOM HTML files as permanent unit test fixtures, added digit collapsing, and introduced MRP plausibility bounds (10% to 200%).
